...
has exactly six entries in category order. Decks with custom `categoryMeta` may
also use an entry `lane` that matches a `categoryMeta` label, such as
`"Rivers"` mapping to the internal `geography` slot.
